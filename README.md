...
**このリポジトリを公開する場合、著作権のある楽曲の MP3 ファイルをコミット・配布してはいけません。**  
`.gitignore` で `*.mp3` などを除外しているので、通常は誤って push されません。手元の `mp3/` にだけ自分で取得した音源を置き、リポジトリに載せるのはソース・`charts/*.json`・`song_db.json`（ファイル名などのメタデータ）に留めてください。
